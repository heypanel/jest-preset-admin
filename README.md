# Jest Test preset for HeyPanel 6 administration unit tests

Default [Jest](https://jestjs.io/) preset for HeyPanel 6 administration development.

```bash
npm install jest @heypanel-ag/jest-preset-sw6-admin @babel/preset-env @babel/plugin-proposal-class-properties --save-dev
```
...
```js
module.exports = {
    preset: '@heypanel-ag/jest-preset-admin',
    globals: {
...
* `setupFilesAfterEnv` - Provides a polyfill for Webpack' `require.context` and sets up the global HeyPanel third-party interface for every test. The object can be accessed using `global.HeyPanel` in test files.
